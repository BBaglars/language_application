...
export default function BlogGenerate() {
  ...
  const [storyTitle, setStoryTitle] = useState('Bir Blog Yazısı');
  const [storyContent, setStoryContent] = useState('Bu bir blog yazısı örneğidir. Blog yazıları genellikle kişisel deneyimler, öneriler veya güncel konular hakkında yazılır.');
          ...
          <View style={[styles.mainContainer, { paddingTop: isWeb ? 32 : 0 }]}>
            ...
            {(showSettings || isWeb) && (
              <View style={[styles.readonlyContainer, isWeb ? styles.webReadonlyContainer : styles.mobileReadonlyContainer, { backgroundColor: isDark ? 'rgba(124,58,237,0.13)' : 'rgba(124,58,237,0.06)', borderColor: isDark ? '#a78bfa' : '#7C3AED', shadowColor: isDark ? '#fbbf24' : '#7C3AED' }]}>
                ...
                <ReadonlyDropdown label="Tür" value="Blog" />
